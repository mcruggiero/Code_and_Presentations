...
def main():
	...
	measurements_directory = "/Users/michaelruggiero/Desktop/reintroductions/" + measurement_folder + "/"
...
def import_jug(jug_name,jug_thickness):
	# Import standard jug
	bpy.ops.import_scene.fbx(filepath='/Users/michaelruggiero/Desktop/reintroductions/jug.fbx')
...
def stack_bounds(stack_number):
	...
	for item in scene.objects:
		if ("GHObj" in item.name) and ('_settled' not in item.name):
...
def animate(stack_number):
	...
	for item in scene.objects:
		...
		elif ("GHObj" in item.name) and ('_settled' in item.name):
			item.select = True
			bpy.context.scene.objects.active = item
			bpy.ops.rigidbody.object_add(type='ACTIVE')
...
def csv_writer(csv,os,data_dicts,measurement_folder):
	directory = "/Users/michaelruggiero/Desktop/reintroductions/"+ measurement_folder
