# 011 What is the greatest product of four adjacent numbers in the same 
# direction (up, down, left, right, or diagonally) in the following 20×20 grid?
#This is FUBAR...the problem is that I came to where the diagonals made a mess
...
def Left(x,y):
	if y > 2:	
		return Grid[x][y] * Grid[x][y-1] * Grid[x][y-2] * Grid[x][y]
	elif y > 1:
		return Grid[x][y] * Grid[x][y-1] * Grid[x][y-2]
	elif y > 0:
		return Grid[x][y] * Grid[x][y-1]
...
for i in range(0, len(Grid)):
	for j in range(0, len(Grid)):
			print( [i, j], Down(i,j), Up(i,j), Right(i,j) )


end = time.clock()
print ("Running time: %s s" % (round(end - start)) )